...
controls = dbc.InputGroup(
    children=[
        ...
        dbc.InputGroup(dbc.Button("Submit", id="submit", style={"marginTop": '5px'})),
    ]
)
...
def run_chatbot(n_clicks, n_submit, user_input, chat_history):
    ...
    try:
        ...
        model_output = f"OpenAI API returned an API Error: {e}"
        pass
    except openai.APIConnectionError as e:
        #Handle connection error here
        print(f"Failed to connect to OpenAI API: {e}")
        model_output = f"Failed to connect to OpenAI API: {e}"
        pass
    except openai.RateLimitError as e:
        #Handle rate limit error (we recommend using exponential backoff)
        print(f"OpenAI API request exceeded rate limit: {e}")
        model_output = f"OpenAI API request exceeded rate limit: {e}"
        pass
